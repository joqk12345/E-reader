...
pub async fn handle_summarize(
    ...
    let provided_count = [args.doc_id.is_some(), args.section_id.is_some(), args.paragraph_id.is_some()]
        .iter()
        .filter(|&&x| x)
        .count();
    ...
    if !matches!(args.style.as_str(), "brief" | "detailed" | "bullet") {
        return Err(ReaderError::InvalidArgument(
            format!("Style must be one of: brief, detailed, bullet. Got: {}", args.style)
        ));
    }
