...
- 建立 existing-document V2 import orchestrator：仅以数据库 document ID 获取后端 source path，串联 archive ingest、package/navigation preparation 和原子 metadata commit；相同 document/hash 重试幂等，不同 source bytes 拒绝静默替换，非 EPUB/缺失 document 在发布 archive 前失败；
- 注册 feature-gated `publication_import_existing_v2` command：DTO 仅接受 `documentId`，flag 关闭不产生文件/DB 副作用，并映射稳定错误码；`publication_open_v2` 有 V2 row 时只读 canonical archive，无 V2 row 时回退 V1 source path，canonical 缺失/损坏不会被可变源路径静默掩盖；
- 建立 policy-v1 XHTML sanitizer 基础层：使用 HTML5 DOM parser 而非 regex，实施输入/node limits、确定性 malformed recovery、script/event/frame/base/form/危险 URL 与 inline SVG 主动内容处理，自动资源 URL 复用 publication allowlist，输出 grouped diagnostics 和 exact/degraded 状态，未知 policy/无效 UTF-8/超限 typed fail-closed；
- 建立 policy-v1 CSS sanitizer 基础层：使用 `cssparser` token/tree boundary，限制 bytes/tokens/nesting，P0 删除全部 `@import`，移除 escaped `expression`/`behavior`/bindings，通过 publication allowlist 验证 URL，并将远程/危险/畸形 URL 中和为 inert fragment；XHTML/CSS 均直接覆盖 active-content EPUB fixture；
- 建立 policy-v1 standalone SVG sanitizer：只输出单一 SVG root，复用 XHTML DOM 安全边界，并用 `cssparser` 校验 SVG presentation URL；移除 script/event/foreignObject/navigation/SMIL animation，限制 bytes/nodes，拒绝多根/非 SVG root，直接覆盖 corpus 安全 SVG fixture；
- canonical V2 session 已接入 sanitized render-resource serving：从 SQLite 加载 committed policy version 与 manifest media map，text/blob 请求均按 XHTML/CSS/SVG parser 输出 session-local cached derived bytes；script 和未分类 active extension 在读取前拒绝，任何 sanitizer 失败返回 `publication.content_refused` 且不 raw fallback。open DTO 明确区分 `sanitized`/`legacyRaw`，同步 size 明确标为 archive uncompressed workload 而非 sanitized bytes；
- V2 import 在 metadata commit 前复用 centralized media classification 和 XHTML/CSS/SVG policies；按 resource/code/count 聚合 diagnostics 并随 publication transaction 写入 `import_reports`，scripted resource 不作为 render content 读取，malformed recovery 可见，sanitizer refusal 不留下 ready metadata rows；
- 从 policy-v1 sanitized spine DOM 生成首批 semantic content blocks：保留 heading/paragraph/list/blockquote/pre/caption/definition/table-row/footnote kind 与顺序，统一 Unicode whitespace，继承 language/direction，生成 publisher-ID 优先或 structural CSS selector、前后各 48 字符 quote context，以及绑定 publication ID/source hash/canonical href 的 Locator V1；不伪造 CFI，block 与 publication metadata 同 transaction，重试不重复；
- 建立同形 Rust/TypeScript `PublicationLocatorV1` DTO 与 validators：严格拒绝未知字段，验证 schema/publication/source/href identity、canonical local href、finite progression/positive position、required anchor 和 quote bounds；semantic block 先通过 typed Rust validator 再序列化，不再拼自由 JSON；
- 注册 identity-only `publication_get_blocks_v2` 与 TypeScript client：仅查询 ready V2 publication，按 spine/block/id canonical order 返回，默认 100、单页最多 200、offset 最多 50,000；Rust 在 serving 前重验 locator JSON 与 denormalized CFI/selector/quote/media fields，TypeScript 再验 response identity、pagination、order、text/locator coherence，corruption fail-closed 为 `publication.locator_invalid`。persistent derived cache、diagnostics UI、CFI/cross-language fixture conformance、selector resolver、search/TTS/translation 接线与前端 import-center UI 尚未接入。
- 建立 `publication_get_position_v2` / `publication_save_position_v2`：阅读位置按 publication 保存完整 Locator 与 overall progression，服务端校验资源/源 hash 身份，并以 `updatedAt` 条件 upsert 防止过期异步写回滚新位置；TypeScript client 拒绝非法响应和非法时间戳。

尚未达到 Phase 0 退出门槛：兼容矩阵尚无端到端通过证据，corpus 仍缺有效内嵌字体与大体积公开样本，性能基线未生成，ADR-001～004 的生产实证条件尚未全部满足，Tauri WebView 恶意 fixture/CSP 跨平台实证和 Blob IPC 性能仍待完成。当前 foliate-js 路径必须保持实验性且默认关闭。
