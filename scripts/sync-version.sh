...
awk -v version="${VERSION}" '
  BEGIN { in_reader = 0; updated = 0 }
  /^\[\[package\]\]$/ { in_reader = 0; print; next }
  /^name = "reader"$/ { in_reader = 1; print; next }
  in_reader && !updated && /^version = "/ {
    print "version = \"" version "\""
    updated = 1
    in_reader = 0
    next
  ...
  END {
    if (!updated) {
      print "failed to update src-tauri/Cargo.lock reader package version" > "/dev/stderr"
      exit 1
    }
  }
