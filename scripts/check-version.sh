...
read_cargo_lock_reader_version() {
  awk '
    BEGIN { pending = 0 }
    /^\[\[package\]\]$/ { pending = 0; next }
    /^name = "reader"$/ { pending = 1; next }
    pending && /^version = "/ {
      gsub(/^version = "/, "", $0)
      gsub(/"$/, "", $0)
      print
...
CARGO_LOCK_VERSION="$(read_cargo_lock_reader_version)"
...
declare -a VERSION_SOURCES=(
  ...
  "src-tauri/Cargo.lock(reader):${CARGO_LOCK_VERSION}"
)
