...
export function ReaderContent() {
  const { paragraphs, isLoading, currentSectionId, bilingualMode, translationDirection } = useStore();
  ...
  const translateSentence = async (paragraphId: string, sentence: string, index: number) => {
    ...
    // 根据设置的翻译方向确定目标语言
    const targetLang = translationDirection === 'en-zh' ? 'zh' : 'en';
  ...
  // 自动翻译当前章节所有句子（开启双语时）
  useEffect(() => {
    if (!bilingualMode || !autoTranslate) return;
  ...
  }, [bilingualMode, autoTranslate, paragraphs, translationDirection, translations, loadingSentences]);
  ...
  }, [translationDirection, bilingualMode]);
                      ...
                      {bilingualMode && (
                        <div className="flex items-center gap-2 ml-4">
                          {loadingSentences.has(key) ? (
                            <p className="text-sm text-blue-600 leading-relaxed">Loading...</p>
                          ) : translations[key] ? (
                            <p className="text-sm text-blue-600 leading-relaxed">{translations[key]}</p>
                          ) : (
                            <button
